...
uses
  ...
  ScktComp, StdCtrls, Mask, ExtCtrls, ComCtrls, jpeg;
...
type
  TForm1 = class(TForm)
    ...
    Image1: TImage;
    Label3: TLabel;
    rb_conected: TRadioButton;
    rb_disconected: TRadioButton;
    GroupBox2: TGroupBox;
    GroupBox3: TGroupBox;
    edt_commandtosend: TEdit;
    Label1: TLabel;
...
begin
  ...
  begin
    ...
    if SocketOpen(true) = false then
      messagedlg('Falha ao conectar em ' + ClientSocket1.Address,
        mterror, [mbok], 0)
    else
    begin
...
//      begin
        rb_conected.Checked := true;
        rb_disconected.Checked := false;
        // timer1.Enabled:=true;
//      end
//      else
//        messagedlg('Falha ao sincronizar relógio: ' + ClientSocket1.Address,
//          mterror, [mbok], 0)
    end;
  end
  else
  begin
    ...
    begin
      rb_conected.Checked := false;
      rb_disconected.Checked := true;
...
// -----------------------------------------------------------------------------
procedure TForm1.btn_sendcommandClick(Sender: TObject);
var
  comando: string;
begin
  comando := edt_commandtosend.Text;
  SendLan(comando, 1000);
end;
// -----------------------------------------------------------------------------
...
begin
  ...
  stop := start + timeout;
  Last := #0;
  while (start < stop) do //and (Last <> ')') do
  begin
    start := gettickcount;
  ...
  if (DataLan <> '') then //and (DataLan[Length(DataLan)] = ')') then
  begin
    if DataLen <> 0 then
    begin
      DataOut := '';
...
begin
  if fl_cmd = 0 then
    comando := '(&A)'
  else if fl_cmd = 1 then
    comando := '(&S)'
  else if fl_cmd = 2 then
    comando := '(&V)';
