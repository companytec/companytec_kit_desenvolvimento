...
begin
  ...
  versao.versao:='5.11.1';
  versao.data:='09/02/2026';
...
begin
  ...
  begin
    comando := AdicionaCheck('(&T' + bico  + modo);
    //messagedlg(comando,mterror,[mbok],0);
    resposta:=Get('LeSTEncerrante2',comando,3000);
    // messagedlg(resposta,mterror,[mbok],0);
    result := resposta;
  end;
end;
...
begin
  par1:=code[1];
  par2:=code[2];
  code_pass.ppu:=strtoint(par1);//4 - strtoint(par1);
  if par2='A' then
      begin
        code_pass.ptotal:=2;
        code_pass.litragem:=2;
      end
  else if par2='E' then
      begin
        code_pass.ptotal:=2;
...
begin
  ...
  if (SAbast[2]='A') or (SAbast[2]='@') then
    ...
    abastecimento.total_dinheiro:=strtofloat(copy(SAbast,2,6-virg.ptotal) + ',' + copy(SAbast,2+(6-virg.ptotal),virg.ptotal));
    abastecimento.total_litros:=strtofloat(copy(SAbast,8,6-virg.litragem) + ',' + copy(SAbast,8+(6-virg.litragem),virg.litragem));
    abastecimento.PU:=strtofloat(copy(SAbast,14,4-virg.ppu) + ',' + copy(SAbast,14+(4-virg.ppu),virg.ppu));
...
begin
  ...
  with abastecimento do
    begin
      ...
      value:=true;
      tipocbc:=SAbast[4];
      total_dinheiro:=strtofloat(copy(SAbast,13,6-virg.ptotal) + ',' + copy(SAbast,13+(6-virg.ptotal),virg.ptotal));
      total_litros:=strtofloat(copy(SAbast,19,6-virg.litragem) + ',' + copy(SAbast,19+(6-virg.litragem),virg.litragem));
      PU:=strtofloat(copy(SAbast,25,4-virg.ppu) + ',' + copy(SAbast,25+(4-virg.ppu),virg.ppu));
...
begin
  ...
  with abastecimento do
    begin
      st_full := SAbast;
      value := true;
      tipocbc := AnsiChar(SAbast[4]);
      total_dinheiro := strtofloat(copy(SAbast,13,6-virg.ptotal) + ',' + copy(SAbast,13+(6-virg.ptotal),virg.ptotal));
      total_litros := strtofloat(copy(SAbast,19,6-virg.litragem) + ',' + copy(SAbast,19+(6-virg.litragem),virg.litragem));
      PU := strtofloat(copy(SAbast,25,4-virg.ppu) + ',' + copy(SAbast,25+(4-virg.ppu),virg.ppu));
      tempo := FormataTempo(copy(SAbast,31,4));
      codbico := copy(SAbast,35,2);
      data := copy(SAbast,41,2) + '/' + copy(SAbast,39,2) + '/20' + copy(SAbast,37,2);
      hora := copy(SAbast,43,2) + ':' + copy(SAbast,45,2) + ':' + copy(SAbast,47,2);
      registro := strtoint(copy(SAbast,49,6));
      encerranteF := strtofloat(copy(SAbast,55,8) + ',' + copy(SAbast,63,2));
      encerranteI := strtofloat(copy(SAbast,65,8) + ',' + copy(SAbast,73,2));
      numbico := strtoint(copy(SAbast,75,2));
      codcombustivel := strtoint(copy(SAbast,77,2));
      numtanque := strtoint(copy(SAbast,79,2));
      try
        voltanque := strtoint(copy(SAbast,81,8));
      except
        voltanque := 0;
      end;
      tag1 := copy(SAbast,89,16);
      tag2 := copy(SAbast,105,16);
      integridade := copy(SAbast,123,2)='00';
      checksum := ComparaCheck(SAbast);
    end;
  result := abastecimento;
end;
//------------------------------------------------------------------------------------------------------------
Function StrToAbastEstendido(SAbast:string):abastEstendido;
var virg:virgula;
    abastecimento: abastEstendido;
begin
  virg:=LeVirgula(copy(SAbast,35,2));
  with abastecimento do
    begin
      st_full:=SAbast;
      value:=true;
      tipocbc:=SAbast[4];
      total_dinheiro:=strtofloat(copy(SAbast,13,8-virg.ptotal) + ',' + copy(SAbast,21-virg.ptotal,virg.ptotal));
      total_litros:=strtofloat(copy(SAbast,21,8-virg.litragem) + ',' + copy(SAbast,29-virg.litragem,virg.litragem));
      PU:=strtofloat(copy(SAbast,29,6-virg.ppu) + ',' + copy(SAbast,35-virg.ppu,virg.ppu));
      tempo:=FormataTempo(copy(SAbast,37,4));
      codbico:=copy(SAbast,41,2);
      data:=copy(SAbast,47,2)+'/'+copy(SAbast,45,2)+'/20'+copy(SAbast,43,2);
      hora:=copy(SAbast,49,2) + ':' + copy(SAbast,51,2);
      registro:=strtoint(copy(SAbast,55,6));
      encerranteF:=strtofloat(copy(SAbast,61,8) + ',' + copy(SAbast,69,2));
      encerranteI:=strtofloat(copy(SAbast,71,8) + ',' + copy(SAbast,79,2));
      numbico:=strtoint(copy(SAbast,81,2));
      codcombustivel:=strtoint(copy(SAbast,83,2));
      numtanque:=strtoint(copy(SAbast,85,2));
      voltanque:=strtoint(copy(SAbast,87,8));
      tag1:=copy(SAbast,95,16);
      tag2:=copy(SAbast,111,16);
      integridade:=copy(SAbast,144,2)='00';
      checksum:=ComparaCheck(SAbast);
    end;
  result:=abastecimento;
end;
...
// Sub-Functions: None                                                                                       -
//------------------------------------------------------------------------------------------------------------
Function ABEstendidoNill():AbastEstendido;
var AbastNull:AbastEstendido;
begin
with AbastNull do
  begin
    value:=false;
...
//------------------------------------------------------------------------------------------------------------
// Função LeAbastecimentoEstendido                                                                           -
// Entrada: Nill                                                                                             -
// Saída: Variável formato abastecimento - função para 8 casas do total e volume e 6 no PPL                  -
// Sub-Functions: None
//------------------------------------------------------------------------------------------------------------
Function LeAbastecimentoEstendido(autoIncrement:boolean=true):AbastEstendido; stdcall; export;
var resposta:string[148];
    comando:string[7];
begin
  try
   comando:='(&A49B)';
   resposta:=Get('LeAbastecimentoEstendido',comando,3000);
   if (length(resposta)=148) then
    begin
      if autoIncrement then Incrementa;
      result:=StrToAbastEstendido(resposta);
      ShowMessage(result.st_full);
    end
    else
    result:=ABEstendidoNill;
  except
    result:=ABEstendidoNill;
  end;
end;
...
// Sub-Functions: None                                                                                       -
//------------------------------------------------------------------------------------------------------------
Function HabilitaIdf(bico:string):integer;stdcall; export;
var comando,resposta:string;
begin
  bico:=copy(bico,1,2);
  if length(bico)=2 then
    begin
      comando:=AdicionaCheck('(&M' + bico + 'H');
      resposta:=Get('AlteraModo',comando,1000);
...
//------------------------------------------------------------------------------------------------------------
// Função DesabilitaIdf                                                                                      -
// Entrada: Nº do bico                                                                                       -
// Saída: Inteiro                                                                                            -
// Sub-Functions: None                                                                                       -
//------------------------------------------------------------------------------------------------------------
Function DesabilitaIdf(bico:string):integer;stdcall; export;
var comando,resposta:string;
begin
  bico:=copy(bico,1,2);
  if length(bico)=2 then
    begin
      comando:=AdicionaCheck('(&M' + bico + 'I');
      resposta:=Get('AlteraModo',comando,1000);
      if resposta='(M'+bico+')' then
        result:=1
      else
...
//------------------------------------------------------------------------------------------------------------
// Função LePPLNivel                                                                                         -
// Entrada: Nº do bico e níveis                                                                              -
// Saída: preço unitário                                                                                     -
// Sub-Functions: None                                                                                       -
//------------------------------------------------------------------------------------------------------------
...
// (Ts19063E0202D3)
// 1234567890123456
function C_ReadCommaCode(nozzle:pchar):pchar;stdcall; export;
var
  comando,resposta,encerrante:string;
begin
  if length(nozzle) = 2 then
  begin
    comando := AdicionaCheck('(&T' + nozzle + 'C');
    resposta := Get('ConsultaEncerrante',comando,3000);
    if length(resposta) = 16 then
    begin
      if (copy(resposta,1,5) = '(Ts' + nozzle) then
      begin
        encerrante := copy(resposta,8,2);
        try
...
//------------------------------------------------------------------------------------------------------------
function C_SetExtendedPriceWithComma(nozzle,level,price: pchar; decimals: byte):integer;stdcall;export;
var
  command,answer:string[75];
  priceSt: string[6];
  priceDb: Double;
begin
  priceSt := PCharToStr(price);
  priceDb := StrToFloat(priceSt);
//  while length(priceSt) < 6 do
//    priceSt := '0' + priceSt;
  if (Length(nozzle) = 2) then //and (Length(priceSt) = 6) then
  begin
    command := AdicionaCheck('(&U' + nozzle + level + '0' + FormataFloat(priceDb,6,decimals));
    answer := Get('AlteraPreco',command,2000);
    if copy(answer,1,4) = '(U' + nozzle then
      result := 1
    else
...
//------------------------------------------------------------------------------------------------------------
Function C_GetSalePAFExtended():pchar; stdcall; export;
var
  answer:string;
  command:string[7];
begin
...
exports
    ...
    C_GetSaleExtended,
    C_SetExtendedPrice,
    C_SetExtendedPriceWithComma,
    C_StopPump,
    C_GetSalePAF,
    C_GetSalePAFExtended,
    C_GetSaleTWC,
    C_AutoPump,
    C_BlockPump,
    C_CloseSerial,
